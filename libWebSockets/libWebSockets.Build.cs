...
public class libWebSockets : ModuleRules
{
	...
	protected virtual string IncludeDirectory
	{
		get
		{
			...
			else if (Target.IsInPlatformGroup(UnrealPlatformGroup.HoloLens))
			{
				return Path.Combine(WebSocketsPackagePath, "include", "Hololens");
			}
	...
	protected virtual string LibraryDirectory
	{
		get
		{
			...
			{
				return Path.Combine(WebSocketsPackagePath, "lib", "Hololens");
			}
			else if (Target.IsInPlatformGroup(UnrealPlatformGroup.Unix))
			{
	...
	public libWebSockets(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		{
			PublicIncludePaths.Add(IncludeDirectory);
			PublicAdditionalLibraries.Add(Path.Combine(LibraryDirectory, "websockets_static.lib"));
			//PublicDelayLoadDLLs.Add(Path.Combine(WebSocketsPackagePath, "bin", "Hololens/websockets.dll"));
			//RuntimeDependencies.Add("$(TargetOutputDir)/websockets.dll", Path.Combine(WebSocketsPackagePath, "bin", "Hololens/websockets.dll"));
